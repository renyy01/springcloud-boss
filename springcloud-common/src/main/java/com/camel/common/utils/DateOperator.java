...
public class DateOperator extends DateParser {
	...
	public static int compareMonthes_1(Date startDate, Date endDate) {
		int intValue=0;
		if ( startDate != null && endDate != null ) {
		BigDecimal big = BigDecimal.valueOf((endDate.getTime() - startDate.getTime()) / 1000f / 60f / 60f / 24f / 31f);
		intValue = big.setScale(0, BigDecimal.ROUND_DOWN).intValue();
		}
		return intValue;
	...
	public static Long compareWeek(Date startDate, Date endDate) {
		...
		long longValue=0L;
		if ( startDate != null && endDate != null ) {
			BigDecimal days=BigDecimal.valueOf((endDate.getTime() - startDate.getTime()) / 1000 / 60 / 60 / 24 / 7);
			longValue=days.setScale(0, BigDecimal.ROUND_UP).longValue();
		}
		return longValue;
/*
